...
def setup_public_network(ipfs_path, logger, node_name):
    ...
    try:
        ...
        # Удаление swarm.key, если он существует
        ...
        # Настройка интервала mDNS (необязательно)
        try:
            result = subprocess.run(
                [ipfs_path, 'config', 'Discovery.MDNS.Interval', '30'],
                capture_output=True, text=True, check=True
            )
            logger.info("PUBLIC_NETWORK: mDNS интервал установлен на 30 секунд")
        except subprocess.CalledProcessError as e:
            logger.warning(f"PUBLIC_NETWORK_WARNING: Не удалось установить Discovery.MDNS.Interval: {e.stderr}. Продолжаем с настройками по умолчанию.")
